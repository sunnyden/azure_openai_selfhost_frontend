...
export class MCPHubServer implements IHubService {
  ...
  private async unregisterClientTools(clientName: string): Promise<void> {
    ...
    const toolsToRemove = Array.from(this.registeredTools).filter(toolName =>
      toolName.startsWith(`${sanitizedClientName}.`)
    );
  ...
  public async listAllTools(): Promise<Map<string, Tool[]>> {
    ...
    for (const [clientName, client] of this.clients.entries()) {
      try {
        ...
        console.warn(`Failed to list tools from client ${clientName}:`, error);
