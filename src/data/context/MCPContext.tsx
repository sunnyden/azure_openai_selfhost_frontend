...
export function MCPProvider({ children }: { children: React.ReactNode }) {
    ...
    async (name: string, command: string, args: string[]) => {
      try {
        ...
        try {
          ...
          await apiClient.mcpHubService.addClient(testConfig, testClientName);
          ...
          if (!tools.has(testClientName)) {
            throw new Error("Server did not register properly with the hub");
          }
          ...
          try {
            await apiClient.mcpHubService.removeClient(testClientName);
          } catch (cleanupError) {
    ...
    if (savedServers) {
      try {
        ...
        if (Array.isArray(parsed)) {
          ...
          console.warn("MCPContext: Invalid saved data format, expected array");
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (isInitialized) {
      console.log("MCPContext: Saving to localStorage, servers:", servers);
      try {
        ...
        console.error("MCPContext: Failed to save to localStorage:", error);
      }
    } else {
  ...
  const importConfigurations = useCallback(
    async (jsonString: string) => {
      try {
        const importedServers = JSON.parse(jsonString) as MCPServerConfig[];

        // Validate the imported data structure
        if (!Array.isArray(importedServers)) {
          throw new Error("Invalid configuration format: expected an array");
        }
